...
<template>
  <v-app>
    <v-container fluid>
      <template>
        <v-data-table v-if="muestra == 0" class="ancho-tabla elevation-15" :headers="headers" :items="ingresos" :search="search" >
          <template v-slot:top>
            <v-toolbar flat >
              <v-toolbar-title style="margin-right:150px">Ingresos</v-toolbar-title>
              <v-spacer></v-spacer>
          ...
          <template v-slot:[`item.actions`]="{ item }">
            ...
            <v-icon  small  class="mr-2" @click="cambioPage(2,item)" v-bind="attrs" v-on="on" >mdi-clipboard-outline </v-icon>
            </template>
            ...
            <template v-if="item.estado">              
              <v-icon small class="mr-2"  @click="activarDesactivarItem(2,item)" v-bind="attrs" v-on="on">mdi-block-helper</v-icon>
            </template>
            <template v-else>
              <v-icon  small  @click="activarDesactivarItem(1,item)"   v-bind="attrs" v-on="on">mdi-check</v-icon>
            </template>
      ...
      <template>
        ...
        <div v-if="muestra==2" class="container pa-4 white grid-list-sm">
          ...
          <template>
            ...
            <v-container class="ventaInfo">
              ...
              <!-- Datos Cliente -->
                ...
                <v-row no-gutters>
                  ...
                  <v-col cols="4">
                    <v-card class="pa-1" outlined >
                      <strong>Direccion: </strong>{{datosCliente.direccion}}                                           
                    </v-card>
                  </v-col>
                </v-row>
                <!-- Datos Vendedor -->
            ...
            <v-row>
              <v-col >
                <v-data-table id="tableIng" class=" elevation-15 "  :headers="articulosVendidos"   :items="articuloIncluido" >
                  <template v-slot:top>
                    <v-toolbar  flat >
                      <v-toolbar-title>Articulos Ingresados</v-toolbar-title>
                    </v-toolbar>
                  </template>
                </v-data-table>
              </v-col >
  ...
  export default {
    data: () => ({      
      ...
      headers: [
        ...
        { text: 'Usuario', value: 'usuario.nombre' ,width:'10%'},
    ...
    methods: {
      ...
      activarDesactivarItem (accion , item) {
        ...
        if(accion == 2){
          ...
          let me = this
          let header = {headers:{"token" : this.$store.state.token}};
          axios.put(`ingreso/desactivar/${id}`, {}, header)
            .then(function(){
              me.listarIngresos();
            })
          ...
          let me = this
          let header = {headers:{"token" : this.$store.state.token}};
          axios.put(`ingreso/activar/${id}`,{},header)
            .then(function(){
              me.listarIngresos();
            })
            .catch(function(error){
      ...
      traerVentaDetalle(item){
        ...
        axios.get(`ingreso/${id}`,header)
          .then(response =>{
            
            this.ventaConDetalleFecha=response.data.compra.createAt
            this.ventaConDetalleEstado=response.data.compra.estado
            this.ventaConDetalleCliente=response.data.compra.persona
            this.ventaConDetalleVendedor=response.data.compra.usuario
            this.ventaConDetalleTipoComp=response.data.compra.tipo_comprobante
            this.ventaConDetalleSerie=response.data.compra.serie_comprobante
            this.ventaConDetalleNumComp=response.data.compra.num_comprobante
            this.ventaConDetalleImpuesto=response.data.compra.impuesto
            this.ventaConDetalleTotal=response.data.compra.total
            this.ventaConDetalleDetalles=response.data.compra.detalles
      ...
      obtenerPersonas(){
        ...
        let header = {headers:{"token" : this.$store.state.token}};
        axios.get("persona/listProveedores/proveedor",header)
          .then(response =>{
